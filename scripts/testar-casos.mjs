...
for (const caso of casos) {
  ...
  console.log(`${ok ? "✓" : "✗"} ${caso.id}: área=${a.area} jec=${a.cabe_juizado_especial} | fundamentos ${conf} confirmados, ${derrub} barrados | fontes ${r.fontes.map((f) => f.id).join(",")} (${seg}s)`);
  if (!ok) console.log(`    esperado: área=${caso.esperado.area} jec=${caso.esperado.cabe_juizado_especial} — ${a.motivo_juizado}`);
  relatorio.push({ id: caso.id, ok, area: a.area, jec: a.cabe_juizado_especial, confirmados: conf, barrados: derrub, sem_base: a.sem_base, fontes: r.fontes.map((f) => f.id), segundos: Number(seg) });
}
